...
import { addCompetenceMonths } from "@/lib/finance/recurrence";
...
const MONTHLY_VIEWS = new Set<FinanceView>([
  "overview",
  "movements",
  "accounts",
  "cards",
...
function competenceHref(params: SearchParams, view: FinanceView, competence: string) {
  const next = new URLSearchParams();
  for (const [key, raw] of Object.entries(params)) {
    if (["competence", "cursor", "success", "error", "request_id"].includes(key)) continue;
    const values = Array.isArray(raw) ? raw : raw ? [raw] : [];
    for (const value of values) next.append(key, value);
  }
  next.set("view", view);
  next.set("competence", competence.slice(0, 7));
  return `/financas?${next.toString()}`;
}
...
export default async function FinancasPage({ searchParams }: PageProps) {
  ...
  const competence = validCompetence(valueOf(searchParams, "competence"));
  ...
  const previousCompetence = addCompetenceMonths(competence, -1).slice(0, 7);
  const nextCompetence = addCompetenceMonths(competence, 1).slice(0, 7);
  const isMonthlyView = MONTHLY_VIEWS.has(view);
  const preservedFormParams = Object.entries(searchParams).flatMap(([key, raw]) => {
    if (["view", "competence", "cursor", "success", "error", "request_id"].includes(key)) return [];
    return (Array.isArray(raw) ? raw : raw ? [raw] : []).map((value) => ({ key, value }));
  });
  ...
  return <main className="min-h-screen bg-slate-50 p-3 sm:p-5 lg:p-8"><div className="mx-auto max-w-7xl space-y-5">
    <header className="overflow-hidden rounded-3xl bg-white shadow-sm"><div className="bg-gradient-to-r from-slate-950 via-slate-900 to-sky-900 p-5 text-white sm:p-7"><MainNav current="financas"/><div className="mt-7 flex flex-col gap-4 md:flex-row md:items-end md:justify-between"><div><p className="text-xs font-semibold uppercase tracking-[.2em] text-sky-300">HERO.FamilyOS · Controle familiar</p><h1 className="mt-2 text-2xl font-semibold sm:text-3xl">Finanças e patrimônio</h1><p className="mt-1 text-sm text-slate-300">{context.family.name} · previsto e realizado em ledger único</p></div>{isMonthlyView ? <div className="flex items-center gap-2"><Link href={competenceHref(searchParams, view, previousCompetence)} aria-label="Visualizar mês anterior" title="Mês anterior" className="grid h-10 w-10 shrink-0 place-items-center rounded-xl border border-white/20 bg-white/10 text-xl font-bold text-white transition hover:bg-white/20">←</Link><form className="flex flex-col gap-2 min-[420px]:flex-row"><input type="hidden" name="view" value={view}/>{preservedFormParams.map(({ key, value }, index) => <input key={`${key}-${index}`} type="hidden" name={key} value={value}/>)}<input type="month" name="competence" defaultValue={competence.slice(0, 7)} aria-label="Competência" className="rounded-xl border border-white/20 bg-white/10 px-3 py-2.5 text-sm text-white [color-scheme:dark]"/><button className="rounded-xl bg-white px-4 py-2.5 text-sm font-semibold text-slate-900">Aplicar</button></form><Link href={competenceHref(searchParams, view, nextCompetence)} aria-label="Visualizar próximo mês" title="Próximo mês" className="grid h-10 w-10 shrink-0 place-items-center rounded-xl border border-white/20 bg-white/10 text-xl font-bold text-white transition hover:bg-white/20">→</Link></div> : <p className="rounded-xl border border-white/15 bg-white/10 px-4 py-2.5 text-sm text-slate-200">Cadastro sem competência mensal</p>}</div></div><div className="p-4"><FinanceNav current={view} competence={competence}/></div></header>
    ...
    {view === "invoices" && <InvoicesView workspace={workspace} competence={competence} canEdit={canEdit}/>}
    {view === "installments" && <InstallmentsView workspace={workspace} competence={competence} canEdit={canEdit}/>}
    ...
    {view === "investments" && <InvestmentsView workspace={workspace} competence={competence} canEdit={canEdit} canAdmin={canAdmin}/>}
