...
function InstallmentFields({ workspace, purchase, categoryId, defaultCompetence }: { workspace: FinanceWorkspace; purchase?: FinanceWorkspace["installments"][number]; categoryId?: string | null; defaultCompetence?: string }) {
  return <>
    ...
    <label className="text-xs text-slate-600">Primeira competência<input name="first_competence" required type="month" defaultValue={purchase?.first_competence.slice(0, 7) ?? defaultCompetence?.slice(0, 7)} className={`mt-1 block w-full ${field}`}/></label>
...
export function InstallmentsView({ workspace, competence, canEdit }: { workspace: FinanceWorkspace; competence: string; canEdit: boolean }) {
  const purchases = workspace.installments.map((purchase) => ({
    purchase,
    installments: workspace.entries
      .filter((entry) => entry.installment_purchase_id === purchase.id && entry.competence === competence)
      .sort((a, b) => (a.installment_number ?? 0) - (b.installment_number ?? 0)),
  })).filter(({ installments }) => installments.length > 0);
  return <div className="space-y-5">
    {canEdit && <FormPanel title="Nova compra parcelada"><DuplicateAmountForm action={createInstallmentPurchase} amountField="total_amount" className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3"><InstallmentFields workspace={workspace} defaultCompetence={competence}/><p className="text-xs text-slate-500 sm:col-span-2 lg:col-span-3">Informe o valor total da compra; o sistema divide esse total pela quantidade de parcelas. Ao selecionar um cartão, a Visão Geral agrupa automaticamente as parcelas nesse cartão.</p><div className="sm:col-span-2 lg:col-span-3"><SaveButton label="Gerar parcelas"/></div></DuplicateAmountForm></FormPanel>}
    <section className={panel}><h2 className="font-semibold">Parcelamentos · {monthLabel(competence)}</h2>{purchases.length ? <div className="mt-4 space-y-3">{purchases.map(({ purchase, installments }) => {
    ...
    })}</div> : <Empty>Nenhuma parcela em {monthLabel(competence)}.</Empty>}</section>
  </div>;
}

export function InvoicesView({ workspace, competence, canEdit }: { workspace: FinanceWorkspace; competence: string; canEdit: boolean }) {
  ...
  return <div className="space-y-5">
    {canEdit && <FormPanel title="Montar fatura"><form action={createInvoice} className="grid gap-3 sm:grid-cols-3">
      ...
      <FieldLabel label="Competência" help="Mês ao qual pertencem as compras e despesas que serão vinculadas à fatura."><input name="competence" type="month" required defaultValue={competence.slice(0, 7)} className={`block w-full ${field}`}/></FieldLabel>
    ...
    <section className={panel}><h2 className="font-semibold">Faturas · {monthLabel(competence)}</h2>{invoices.length ? <div className="mt-4 space-y-3">{invoices.map((invoice) => {
    ...
    })}</div> : <Empty>Nenhuma fatura montada em {monthLabel(competence)}.</Empty>}</section>
  </div>;
}
