...
export function InvoicesView({ workspace, canEdit }: { workspace: FinanceWorkspace; canEdit: boolean }) {
  ...
  return <div className="space-y-5">
    ...
    <section className={panel}><h2 className="font-semibold">Faturas</h2>{workspace.invoices.length ? <div className="mt-4 space-y-3">{workspace.invoices.map((invoice) => {
      ...
      const entries = workspace.entries.filter((entry) => entry.card_invoice_id === invoice.id && entry.source_key !== `invoice-payment:${invoice.id}` && entry.source_key !== `invoice-payment-reversal:${invoice.id}`);
      ...
      return <article key={invoice.id} className={`rounded-xl border p-4 ${isPaid ? "border-emerald-300 bg-emerald-50/50" : ""}`}>
        ...
        {canEdit && <div className="mt-4 flex flex-wrap items-end gap-3">{invoice.status === "open" && <form action={closeInvoice}><input type="hidden" name="id" value={invoice.id}/><input type="hidden" name="closed_amount" value={invoice.expected_amount}/><input type="hidden" name="closing_date" value={today}/><ConfirmSubmitButton label="Fechar fatura" confirmMessage="Confirma o fechamento desta fatura?" className={primary}/></form>}
