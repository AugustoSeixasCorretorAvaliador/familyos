    ...
    <section className={panel}>
      ...
      {orderedRecurrences.length ? <div className="mt-4 space-y-3">{orderedRecurrences.map((item, index) => <Fragment key={item.id}>
        ...
        <article className="rounded-xl border p-4">
          ...
          {canEdit && <FormPanel title="Editar recorrência"><form action={updateRecurrence} className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3"><input type="hidden" name="id" value={item.id}/><input type="hidden" name="apply_from_competence" value={competence.slice(0, 7)}/>{fieldsFor(item)}<label className="flex items-start gap-3 rounded-xl border border-sky-200 bg-sky-50 p-3 text-sm text-slate-700 sm:col-span-2 lg:col-span-3"><input type="checkbox" name="apply_to_future_unpaid" value="true" className="mt-0.5 h-4 w-4 rounded border-slate-300"/><span><strong className="block text-slate-900">Aplicar também aos lançamentos futuros ainda não pagos</strong>Atualiza, a partir de {competence.slice(0, 7)}, descrição, valor, natureza, categoria, tipo, conta, cartão e responsável. Lançamentos realizados ou pagos permanecem preservados.</span></label><p className="text-xs text-slate-500 sm:col-span-2 lg:col-span-3">Sem marcar a opção, salvar altera somente a regra para as próximas gerações. Datas de lançamentos já gerados não são modificadas.</p><div className="sm:col-span-2 lg:col-span-3"><SaveButton/></div></form></FormPanel>}
        </article>
